    ...
    libraryDependencies ++= {
      if(alpnApiIncluded.value) {
        Seq(
          ...
          "org.mortbay.jetty.alpn" % "jetty-alpn-agent" % alpnAgentVersion.value % alpnConfig
        )
      } else {
        Seq("org.mortbay.jetty.alpn" % "jetty-alpn-agent" % alpnAgentVersion.value % alpnConfig)
      }
    },
